...
"""
run with python2.7 
create samples of negative data and k-shuflle of them, for all 17 species, and for all values of k,
from k=1 to the MAXIMAL_K=9 value.
"""
...
sys.path.insert(0, base_path[:-len('/negative_data_vs_k_shuffle')])
import ushuffle
sys.path.insert(0, base_path[:-len('/negative_data_vs_k_shuffle')]+'/CNN/')
import data_handle
...
samples_out_base_dir_for_npy_files = "/cs/cbio/dikla/projects/negative_data_vs_k_shuffle/data/npy_files/"

samples_out_base_dir_for_text_files =  "/cs/cbio/dikla/projects/negative_data_vs_k_shuffle/data/samples/"
...
def translate_bases_from_original_sample(letter):
    ...
    # if base is not A,G,C or T, we print an error message and exit.
    else:
...
def create_positive_or_negative_samples(species, is_positive, k=None):
    ...
    samples_input_base_dir = "/cs/cbio/dikla/projects/H3K27ac_vs_k_shuffle/data/new_negative_data/final_negative_data/"
    samples_file_path = os.path.join(samples_input_base_dir, species + "_final_samples_only_bases_without_N.txt")
