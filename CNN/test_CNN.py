...
def get_map_model_ids_species(project, train_file_path, model_ids):
    ...
    with open(train_file_path) as train_file:
        for line in train_file:
            ...
            if len(map_model_ids_species) == len(project.species):
                break
...
def write_labels_and_scores(output_dir, best_model_validation_id, array_prediction_scores,
                            array_true_labels, test_species):
    out_score_path = os.path.join(output_dir, test_species+"_scores_" +
                                  str(best_model_validation_id) + ".txt")
    out_true_labels_path = os.path.join(output_dir, test_species+"_labels_" +
                                        str(best_model_validation_id) + ".txt")
    ...
    if project.project_name != "simulated_data":
        if project.k:
            ...
            test_x_path = os.path.join(project.base_dir_data_path,
                                       str(project.num_times_negative_data_is_taken)+"_times_negative_data",
                                       "npy_files",
                                       test_species, 'test_X.npy')
            test_y_path = os.path.join(project.base_dir_data_path,
                                       str(project.num_times_negative_data_is_taken)+"_times_negative_data",
                                       "npy_files",
                                       test_species, 'test_Y.npy')
